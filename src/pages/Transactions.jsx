        ...
        <div className="space-y-6">
            ...
            <div className="bg-white border border-gray-200 overflow-hidden">
                        ...
                        <thead>
                            <tr className="border-b border-gray-200 bg-gray-50/50">
                                <th className="w-10 p-4">
                                    <input type="checkbox" className="border-gray-300 text-black focus:ring-black" />
                                </th>
                                <th className="text-left text-xs font-medium text-gray-500 uppercase tracking-wider p-4">Deal ID</th>
                                <th className="text-left text-xs font-medium text-gray-500 uppercase tracking-wider p-4">Type</th>
                                <th className="text-left text-xs font-medium text-gray-500 uppercase tracking-wider p-4">Counterparty</th>
                                ...
                                <th className="text-right text-xs font-medium text-gray-500 uppercase tracking-wider p-4">Rate</th>
                                <th className="text-left text-xs font-medium text-gray-500 uppercase tracking-wider p-4">Value Date</th>
                                ...
                                <tr key={idx} className="hover:bg-gray-50 group transition-colors">
                                    ...
                                    <td className="p-4">
                                        <div className="flex items-center gap-2">
                                            <span className={`p-1 ${row.dir === 'Buy' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
                                                {row.dir === 'Buy' ? <ArrowDownLeft size={14} /> : <ArrowUpRight size={14} />}
                                            </span>
                                            <span className="text-sm text-gray-700">{row.type}</span>
                                        </div>
                                    </td>
                                    <td className="p-4 text-sm text-gray-700">{row.cp}</td>
                                    <td className="p-4 text-sm text-gray-700 font-mono">{row.ccy}</td>
                                    <td className="p-4 text-sm text-right font-medium text-gray-900">{row.amt}</td>
                                    <td className="p-4 text-sm text-right text-gray-500 font-mono">{row.rate}</td>
                                    <td className="p-4 text-sm text-gray-500">{row.date}</td>
                                    <td className="p-4">
                                        <span className={`inline-flex items-center px-2 py-0.5 text-xs font-medium border ${row.status === 'Settled' ? 'bg-green-50 text-green-700 border-green-200' :
                ...
                <div className="px-4 py-3 border-t border-gray-200 flex items-center justify-between">
                    <p className="text-xs text-gray-500">Showing 6 of 128 transactions</p>
                    <div className="flex gap-2">
                        <button className="px-3 py-1 text-xs font-medium text-gray-600 border border-gray-200 hover:bg-gray-50">Previous</button>
                        <button className="px-3 py-1 text-xs font-medium text-gray-600 border border-gray-200 hover:bg-gray-50">Next</button>
                    </div>
                </div>
            </div>
        </div>
